...
宗量：方法的接受者与方法的参数统称为方法的宗量<br/>
当前java在编译阶段，既要看静态类型，又要看方法参数，所以静态分派属于多分派类型；<br/>
由于编译器已决定目标方法签名，所以此时虚拟机只考虑接受者的实际类型，故动态分派属于单分派类型
...
## 动态类型语言支持
什么是动态语言？类型检查的主体过程是在运行期而非编译期
- java.lang.invoke包<br/>
```java
...
public class Demo{
  ...
  public static void println(MethodHandle mh, String str) throws Throwable{
    mh.invokeExact(str);
  }

  public static void main(String[] args) throws Throwable{
    Object obj = new ClassA();
    Demo.println(getPrintlnMH(obj), "test");
  }
...
```
MethodType:代表方法类型,包含了方法返回值，以及第一个参数及以后的参数<br/>
lookup:指定类中查找符合给定的方法名 方法类型，并且符合调用权限的方法句柄<br/>
getPrintlnMH模拟了invokevirtual指令的执行过程，返回值MethodHandle相当于调用方法的一个引用<br/>
